...
|-----------|-----------|---------------|-----------------|
| Accuracy | "A screenshot of a website with a form" (no application identified) | "NTRT New Term Request Form in Internet Explorer with 5 dropdown fields all showing UNASSIGNED, Submit button, VistA branding, and VA seal in sidebar" (key elements correct) | All form field labels exact (SUBJECT MATTER DOMAIN, ROLE, SETTING, SERVICE, DOCUMENT TYPE), all showing UNASSIGNED, sidebar links enumerated (Allergy, Clinical Doc Title *, Vitals, Search, New User, Edit User), nav bar links exact, footer date "August 14th, 2006", window title exact |
| Specificity | "A blue and white form page" (no details read) | "Five dropdown fields labeled SUBJECT MATTER DOMAIN, ROLE, SETTING, SERVICE, and DOCUMENT TYPE; red Submit button; sidebar has Request and Admin sections" (reads major labels) | All field labels with dropdown widths noted, DOCUMENT TYPE asterisk for required, COMMENTS text area with scrollbar, Help button position, << Back link, VistA logo subtitle text, red vs blue color coding, IE toolbar button descriptions, footer link list complete |
| Completeness | Describes the form but misses sidebar, header, or footer | Header with NTRT title, form fields, sidebar navigation, Submit button, and footer date described | Window chrome (title bar, menu, toolbar), header (VistA logo, NTRT title, red nav bar with 3 links), sidebar (2 sections with all links, VA seal), form (title, required field note, all 5 dropdowns, Comments area, Help button, Back link, Submit button), footer (7 links, date) |
| Usefulness | "A VA government form" (no system identification) | "NTRT terminology request form from the VA CPRS TIU manual; used for requesting new clinical document types in VistA" | Identifies NTRT as clinical terminology standardization tool, explains relationship to TIU/CPRS/VistA, notes the form's purpose (requesting new document definitions), connects to the manual's documentation of clinical workflow, places in mid-2000s VA health IT context from the footer date |
