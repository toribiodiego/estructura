...
|-----------|----------------|-----------------|---------------------|
| Correctness | Misidentifies the subject as "AI model performance" or "benchmark results" (wrong domain -- these are occupation usage shares). Reading "37.2%" as "37%" is a minor imprecision; fabricating a 9th panel or inventing a category not shown triggers the hallucination cap. Stating all categories are declining (3 are rising) is a trend-direction error. | All 8 SOC category names correct and matched to the right panels. Most of the 24 labeled values accurate. May misread one value (e.g., "9.4%" as "9.5%") or miss that y-axis scales differ across panels. Trend directions correct for at least 6 of 8 panels. | All 24 labeled values exact (37.2/39.6/36.9, 10.2/9.4/8.5, 9.3/11.0/12.7, 7.8/7.0/8.4, 6.3/6.8/7.4, 5.9/4.4/3.1, 4.7/3.8/2.5, 4.5/3.1/2.7). All 8 trend directions correct (Computer/Math: rise-then-fall; Arts: steady decline; Educational: steady rise; Office: dip-then-rise; Life/Physical: steady rise; Business: steady decline; Architecture: steady decline; Management: steady decline). Notes y-axis scales differ per panel. Tolerance: zero for labeled percentages. |
| Information Recovery | Recovers fewer than 12 of the 24 labeled data points. A search for "Educational Instruction 12.7%" or "Architecture and Engineering 2.5%" would fail. May name only 2-3 of the 8 categories. | Recovers all 8 category names and most labeled values (~18-20 of 24). Captures the overall title and time range (Jan-Aug 2025). A search for "Computer and Mathematical 39.6%" matches. May miss the caption text or the V1/V2/V3 reference. | All 24 labeled percentages recovered across all 8 panels. Chart style (orange line with tan area fill), differing y-axis scales, the overall title "Usage share trends across economic index reports (V1 to V3)", caption reference to "sampled conversations on Claude.ai", and all 3 x-axis time points (Jan 2025, Mar 2025, Aug 2025) captured. A search for any category-timepoint-value triple returns a match. Parity met. |
| Retrieval Value | "Charts showing percentages over time" with no category names, no values, no document context. Not self-contained; depends on surrounding text to convey meaning. | Self-contained summary naming all 8 categories with key values and trend directions. Searchable for "Claude.ai occupation usage" or "Computer and Mathematical 39.6%". Missing V1/V2/V3 context and SOC classification link. | Self-contained annotation embedding SOC classification context, V1/V2/V3 report version meaning, all 24 values, and the Anthropic Economic Index provenance. Findable by any category name + percentage, by "occupation usage share trends", or by "economic index V1 V2 V3". Domain vocabulary (SOC major groups, usage share) embedded in natural prose. |
...
| Dimension | Score 40 (poor) | Score 70 (good) | Score 95 (excellent) |
|-----------|----------------|-----------------|---------------------|
| Correctness | Calls it a "horizontal bar chart" (it is vertical) or says "20 countries" (there are 30). Reading "21.6%" as "22%" is a minor imprecision; claiming China appears in the chart triggers the hallucination cap (China is absent). Saying "India is close behind the US" misrepresents the 3x gap (21.6% vs 7.2%). | Correct chart type (vertical bar), correct count (30 countries), US at 21.6% and India at 7.2% both accurate. Most labeled values correct. May misattribute one country name due to rotated labels. Notes the gradient coloring. | All 30 country-value pairs exact (US 21.6%, India 7.2%, Brazil/Japan/Korea 3.7% three-way tie, ... through South Africa 0.5%). Notes all multi-way ties (3.7% x3, 1.1% x4, 1.0% x4). Caption scope correctly identified as "Claude.ai Free and Pro conversations" only. Tolerance: zero for all labeled percentages. |
| Information Recovery | Recovers fewer than 10 of the 30 country-value pairs. A search for "Korea 3.7%" or "Philippines 0.9%" would fail. May name only the US bar. | Recovers the top 10 countries with values and mentions the long-tail shape. A search for "United States 21.6%" or "Brazil 3.7%" matches. May miss mid-ranked countries (ranks 12-25) and the tie patterns. | All 30 country names in rank order with all 30 percentage values. The chart title, y-axis label ("Share of global usage (%)"), gradient bar coloring, and caption text recovered. Tie groups explicitly noted. Distribution shape (steep US drop-off, gradual long tail) described. A search for any country + value pair returns a match. Parity met. |
| Retrieval Value | "Bar chart about countries" with no country names, no values, no document context. Not self-contained. | Self-contained summary naming the top countries with values and the Anthropic Economic Index source. Searchable for "Claude usage by country" or "United States 21.6%". Missing the Free/Pro scope qualifier and population-size caveat. | Self-contained annotation embedding all 30 countries, the Chapter 2 geographic analysis context, the Free/Pro scope, and the population-size caveat noted in surrounding text. Findable by any country name + percentage, by "global Claude adoption", or by "Anthropic Economic Index geographic distribution". |
...
| Dimension | Score 40 (poor) | Score 70 (good) | Score 95 (excellent) |
|-----------|----------------|-----------------|---------------------|
| Correctness | Describes it as "total usage" or "number of users" map (it is per working-age capita). Claiming China is in the "Emerging" tier is a hallucination -- China appears gray (Claude not available). Assigning exact AUI numeric values to countries from the map triggers the hallucination cap (map shows tiers only, not numbers). Saying "5 categories" instead of 7 is a count error. | Correctly identifies the 7-tier system and the per-capita normalization. US, Canada, Australia correctly placed in Leading tier. China correctly noted as gray (not available). Regional patterns broadly correct. May slightly misidentify one small country's tier. | All 7 legend categories named with percentile definitions (Leading = top 25%, Upper middle = 50-75%, Lower middle = 25-50%, Emerging = bottom 25%, plus Minimal, Claude not available, No data). Confidently identified countries placed correctly without overclaiming small/ambiguous ones. Caption's "per working-age capita" qualifier noted. Footnote references 5,6 captured. |
| Information Recovery | Recovers "a world map with colors" but names fewer than 3 of the 7 legend categories. A search for "Leading (top 25%)" or "Claude not available" would fail. No country-tier assignments mentioned. | All 7 legend categories named with color descriptions. Regional patterns for 3-4 continents described. Key country-tier assignments covered (US, Canada, Japan, Australia as Leading; China as not available). A search for "Anthropic AI Usage Index tiers" matches. May miss the caption text or percentile definitions. | All 7 tiers with percentile definitions and color descriptions. Regional patterns for all continents (North America/Europe/Oceania leading, Africa mostly Emerging/Minimal, China not available). Specific identifiable countries assigned to tiers. Chart title, legend title, full caption text with "per working-age capita", legend position (bottom-left), and map projection noted. Parity met. |
| Retrieval Value | "A colorful world map" with no tier names, no country assignments, no document context. Not self-contained. | Self-contained summary naming the 7 tiers and key country-tier assignments. Searchable for "AI Usage Index tiers" or "Claude adoption by country". Missing per-capita methodology context and table 2.1 reference. | Self-contained annotation embedding the AUI methodology (per working-age capita normalization, quartile-based tiers), regional patterns, Chapter 2 provenance, and the caveat that numeric AUI values are in Table 2.1 not the map. Findable by "choropleth Claude adoption", "Leading tier countries", or "Anthropic AI Usage Index". |
...
| Dimension | Score 40 (poor) | Score 70 (good) | Score 95 (excellent) |
|-----------|----------------|-----------------|---------------------|
| Correctness | Describes it as a "linear regression" without noting the power-law/log-log context. Reporting R-squared as "0.71" instead of the labeled 0.709 is an imprecision error. Claiming China appears on the chart triggers the hallucination cap (CHN is absent -- Claude not available). Stating "all countries follow the regression line" ignores clear outliers (ISR above, QAT/SAU/KWT/OMN below). | Power-law fit correctly identified (AUI ~ GDP^0.69). Statistics box values accurate (beta=0.690, R-sq=0.709, p<0.001). ISR correctly identified as highest-AUI outlier. Log-scale axes noted. May misidentify one ISO code in the dense middle region. | All labeled statistics exact: beta=0.690, p<0.001, R-squared=0.709, "Power law: AUI ~ GDP^0.69". Both axis labels with units correct. ISR as highest AUI, Gulf states (QAT, SAU, KWT, OMN) below line, GEO as positive outlier all noted. Color bar correctly described as redundant encoding of ln(AUI). No overclaiming precise x,y coordinates for dense-cluster points. Tolerance: zero for all labeled text/statistics. |
| Information Recovery | Recovers "a scatter plot with a trend line" but omits the statistics box, the power-law equation, axis labels, and country identifications. A search for "beta 0.690" or "AUI ~ GDP^0.69" would fail. | Statistics box values, both axis labels, the regression equation, and 10+ country positions recovered. A search for "R-squared 0.709" or "Israel highest AUI" matches. May miss the color bar, the caption's 200-observation threshold, or specific outlier clusters. | All labeled statistics (beta, R-squared, p-value, power-law equation), both axis labels with units, color bar range and label, dashed regression line style, caption text including 200-observation minimum, ISO code labels on visible points, outlier clusters (ISR above, Gulf below, GEO positive outlier, African countries lower-left), and the redundant color encoding noted. Parity met. |
| Retrieval Value | "Chart about countries and money" with no axis labels, no statistics, no document context. Not self-contained. | Self-contained summary citing the power-law relationship and key outliers. Searchable for "income vs AUI" or "R-squared 0.709". Missing the sublinear growth interpretation and the Gulf state contextual explanation. | Self-contained annotation embedding the power-law interpretation (exponent 0.69 means sublinear: doubling GDP increases AUI by ~61%), the Gulf state anomaly context (high income, low AUI), Israel's tech-sector concentration, the 200-observation threshold, and Chapter 2 provenance. Findable by "AUI ~ GDP^0.69", "Israel AUI outlier", or "income Claude adoption power law". |
...
|-----------|-----------|---------------|-----------------|
| Correctness | "Four scatter plots about different jobs and AI" (no statistics, wrong axis descriptions). Claiming all four panels show significant trends is factually wrong -- only Office & Administrative has p<0.05. Inventing country labels on the bubbles triggers the hallucination cap (no ISO codes appear on these scatter plots, unlike V06). Misreading beta=-0.700 as positive reverses the Computer & Mathematical trend direction. | All 4 panel titles correct. Main directional finding captured: Computer & Mathematical negative slope, Office & Administrative positive. Statistics for the significant panel (beta=0.448, p=0.009, R-sq=0.091) accurate. May misread one statistic in the non-significant panels. Notes that relationships are "noisy." | All 12 statistics exact: Computer & Mathematical (beta=-0.700, p=0.385, R-sq=0.007), Educational Instruction & Library (beta=0.082, p=0.858, R-sq=0.000), Arts/Design/Entertainment/Sports/Media (beta=0.357, p=0.146, R-sq=0.037), Office & Administrative Support (beta=0.448, p=0.009, R-sq=0.091). Correctly distinguishes the one significant panel from three non-significant. Notes AUI axis defined as "usage % / working-age population %". Tolerance: zero for all labeled statistics. |
| Information Recovery | Recovers "multi-panel scatter plots" but names fewer than 2 of the 4 SOC groups. A search for "beta -0.700" or "Office and Administrative Support p=0.009" would fail. Bubble encoding (size and color) not described. | All 4 SOC group names recovered with regression statistics for 2-3 panels. Bubble size (usage count) and color (AUI tier) encoding noted. A search for "Computer and Mathematical beta -0.700" matches. May miss the size legend values (100/1,000/10,000) or the 5-tier color category names. | All 4 panel titles verbatim, all 12 statistics, both dual encodings (5-tier color categories named; 3 size reference values 100/1,000/10,000), axis labels with units, dashed regression line style, caption text including methodology (200-observation threshold, equal-weight regression, O*NET task mapping), and the overall title. Parity met. |
| Retrieval Value | "Chart about jobs and AI usage" with no panel names, no statistics, no document context. Not self-contained. | Self-contained summary naming all 4 occupation groups with key statistics and the main finding (programming share decreases, office/admin share increases with adoption). Searchable for "occupation group share vs AUI" or "Computer and Mathematical beta". Missing the significance distinction and SOC/O*NET context. | Self-contained annotation embedding the diversification thesis (programming-dominant to broader usage at higher adoption), the critical significance distinction (3 of 4 panels non-significant), SOC/O*NET framework context, and Chapter 2 provenance. Findable by any panel name + statistic, by "occupation diversification AI adoption", or by "Figure 2.7 Anthropic Economic Index". |
...
| Dimension | 40 (poor) | 70 (adequate) | 95 (excellent) |
|-----------|-----------|---------------|-----------------|
| Correctness | "Four panels showing different countries and what they use AI for" (no multipliers, no specific categories). Swapping Vietnam's and India's lists is a country-attribution error (they share overlapping categories but with different multipliers). Inventing a 6th row for any panel triggers the hallucination cap (each panel has exactly 5). Reading Brazil's 1.01x (game development) as "1.1x" is an imprecision error. Paraphrasing cluster labels loses the exact wording that distinguishes overlapping categories. | All 4 countries correctly assigned to panels. Main patterns accurate: US non-technical, Brazil translation/legal dominant, Vietnam and India software-heavy. Most multipliers correct; may misread one value or truncate one description. Notes that Brazil's range (1.01x-6.4x) is much wider than US's (1.29x-1.43x). | All 20 request descriptions verbatim with all 20 multipliers at stated precision (6.4x, 5.0x, 1.43x, 2.1x, etc.). Country-panel assignments all correct. Overlapping categories between Vietnam and India distinguished (e.g., "Fix and improve web and mobile application UI layouts" appears in both at 1.70x vs 2.4x). Multiplier ranges per country noted. Tolerance: zero for all labeled text and values. |
| Information Recovery | Recovers fewer than 8 of the 20 request-multiplier pairs. A search for "translation services 6.4x" or "cooking nutrition 1.43x" would fail. May name only 1-2 countries. | All 4 country names and panel positions recovered. 12-16 of the 20 request descriptions with multipliers captured. A search for "Brazil translation 6.4x" or "India UI layouts 2.4x" matches. May miss bottom rows of some panels or the caption methodology text. | All 20 request descriptions with multipliers, all 4 country panels in correct layout position (US top-left, Brazil top-right, Vietnam bottom-left, India bottom-right). Title text, caption text including methodology (middle granularity, 1% frequency threshold), card design elements (salmon banner, pink background, alternating rows). A search for any country + request + multiplier triple returns a match. Parity met. |
| Retrieval Value | "Chart comparing AI use in different countries" with no country names, no request types, no multipliers. Not self-contained. | Self-contained summary naming all 4 countries with 2-3 key request types each and the main pattern (US non-technical, India/Vietnam software, Brazil translation/legal). Searchable for "overrepresented requests United States" or "Brazil legal assistance 5.0x". Missing methodology context. | Self-contained annotation embedding the overrepresentation metric definition (country share vs global share), the contrast between US diversified non-technical usage and South/Southeast Asian software concentration, Brazil's extreme translation/legal specialization, the 1% frequency threshold and middle-granularity methodology, and Chapter 2 provenance. Findable by any country + request combination, by "geographic AI specialization", or by "Figure 2.8 Anthropic Economic Index". |
...
| Dimension | 40 (poor) | 70 (adequate) | 95 (excellent) |
|-----------|-----------|---------------|-----------------|
| Correctness | "Two charts comparing Claude and API usage" (no Gini values, wrong chart type descriptions). Swapping which platform has 0.842 vs 0.822 Gini is a value-attribution error (1P API = 0.842, Claude.ai = 0.822). Swapping the 10.5% and 12.7% bottom-80% values is the same kind of error. Claiming task names are visible on the scatter plot triggers the hallucination cap (no task labels appear). Describing the Perfect Equality diagonal as a "regression line" misidentifies a theoretical reference. | Left panel correctly identified as Lorenz curves; Gini values (0.842/0.822) correctly attributed per platform. Right panel identified as log-log scatter with power-law fit. Bottom-80% annotation values correct (10.5% 1P API, 12.7% Claude.ai). May misread one regression coefficient or miss that both axes are ln-transformed. | All labeled statistics exact: Gini 0.842 (1P API) and 0.822 (Claude.ai); bottom-80% 10.5% (1P API) and 12.7% (Claude.ai); regressions y=-1.11x+2.51 (1P API), y=-1.13x+2.53 (Claude.ai), y=-1.00x+2.52 (Zipf's Law). Notes both slopes steeper than Zipf's -1.00. Both axes identified as ln-scale. Dashed vs solid line styles distinguished. Tolerance: zero for all labeled values. |
| Information Recovery | Recovers "two panels with curves" but omits Gini coefficients, regression equations, and the annotation box. A search for "Gini 0.842" or "Zipf's Law y = -1.00x" would fail. | Both panels with subtitles, both Gini values, the annotation box text, and 2 of 3 regression equations recovered. A search for "bottom 80% of tasks 10.5%" or "1P API Gini 0.842" matches. May miss one regression equation, the highlighted data points at 80%, or the caption methodology. | All 6 legend entries across both panels, all labeled statistics, annotation box text verbatim, highlighted data points at 80% mark with color-matched labels, both axis labels with units per panel, panel subtitles, line styles (dashed/solid), caption text including methodology (0.1% usage threshold, O*NET tasks). Parity met. |
| Retrieval Value | "Charts about how people use AI" with no chart type, no statistics, no document context. Not self-contained. | Self-contained summary identifying both chart types (Lorenz curve, log-log power-law) with key statistics. Searchable for "Lorenz curve Claude.ai Gini" or "power law task concentration". Missing the Zipf's Law interpretation and the O-Ring theory reference. | Self-contained annotation embedding the Gini interpretation (extreme concentration comparable to most unequal economies), the bottom-80%/10-13% finding, the super-Zipfian concentration interpretation (slopes steeper than -1.00), the near-identical platform distributions, and Chapter 3 provenance. Findable by "Gini coefficient task concentration", "Zipf's Law Claude usage", or "Lorenz curve O*NET tasks". |
...
| Dimension | 40 (poor) | 70 (adequate) | 95 (excellent) |
|-----------|-----------|---------------|-----------------|
| Correctness | "Two scatter plots comparing automation and augmentation for AI" (no percentages, panels not attributed to platforms). Swapping which panel is 1P API vs Claude.ai inverts the entire finding. Calling the diagonal a "regression line" or "trend line" is wrong -- it is a reference line where automation share equals augmentation score. Saying 1P API has "no" augmentation dominant tasks ignores the labeled 2.1%. Claiming task names are visible triggers the hallucination cap (no task labels appear). | Left panel correctly identified as 1P API (97.9% automation dominant); right as Claude.ai (47.0%/53.0% split). Diagonal correctly described as a reference line. Distinguishes the asymmetric axis labels ("Automation Share" vs "Augmentation Score"). The dramatic cross-panel contrast captured. May round 97.9% to "about 98%." | Exact legend percentages: 1P API 97.9%/2.1%, Claude.ai 47.0%/53.0%. Both panels correctly attributed. Axis labels distinguished precisely ("Automation Share (%)" on x, "Augmentation Score (%)" on y). Diagonal identified as a reference line, not a fit. Privacy-preserving zero caveat from caption noted. No task name fabrication. Tolerance: zero for all labeled percentages and text. |
| Information Recovery | Recovers "two scatter plots with colored dots" but omits platform attributions and the legend percentages. A search for "97.9% automation dominant" or "Augmentation Score" would fail. | Both panel titles (1P API, Claude.ai), both legend breakdowns, both axis labels, and the diagonal reference line recovered. A search for "1P API 97.9% automation dominant" matches. May miss the spatial distribution differences (1P API strip at y=0, Claude.ai broad fill) or the privacy-preserving methodology. | Both panel subtitles, both axis labels with exact text, both legends with exact percentages, diagonal reference line style, bubble size variation (no explicit legend noted), spatial distribution per panel (1P API sparse with y=0 strip, Claude.ai dense triangular fill), caption text including privacy-preserving methodology and O*NET task reference, grid lines. Parity met. |
| Retrieval Value | "Chart about AI automation" with no platform names, no percentages, no document context. Not self-contained. | Self-contained summary naming both platforms with their automation/augmentation splits and the core finding (API nearly all automation, chat roughly split). Searchable for "automation dominant 97.9%" or "Claude.ai augmentation 53%". Missing the economic implications context. | Self-contained annotation embedding the automation/augmentation definitions (substitution vs complementarity), the dramatic API-vs-chat modality difference, the economic implications (automation displaces tasks, augmentation enhances productivity), the O*NET framework, and Chapter 3 provenance. Findable by "automation augmentation Claude.ai vs API", "97.9% automation dominant", or "Figure 3.5 Anthropic Economic Index". |
...
| Dimension | 40 (poor) | 70 (adequate) | 95 (excellent) |
|-----------|-----------|---------------|-----------------|
| Correctness | "Scatter plot of cost vs usage with a downward trend line" (describes axes as raw values instead of residuals). Misreading R-squared as 0.25 instead of 0.025 overstates the relationship by 10x. Describing this as a "strong negative correlation" contradicts the visible diffuse cloud and the 0.025 R-squared. Claiming task names are visible triggers the hallucination cap (no labels on points). Confusing ln-scale with linear scale changes the elasticity interpretation. | R-squared = 0.025 correct. Both axes identified as residual (partialled-out) values. Negative slope direction correct. Describes the relationship as weak. May omit the ln-transformation qualifier or miss the elasticity from the caption. | R-squared = 0.025 exact from legend. Both axes identified as "Residual ln(API Cost Index)" and "Residual ln(Usage share (%))" verbatim. Elasticity = -0.29 from caption correctly cited. Control variables named (occupational category fixed effects, collaboration mode share, privacy-censoring indicators). Notes the visual appearance matches the weak R-squared (diffuse cloud, no obvious pattern). Tolerance: zero for R-squared; caption-sourced elasticity reported as stated. |
| Information Recovery | Recovers "a scatter plot with a trend line" but omits R-squared, axis labels, and the partial regression context. A search for "R-squared 0.025" or "elasticity -0.29" would fail. | Title (both lines), both axis labels, R-squared from legend, and the negative slope direction recovered. A search for "partial regression R-squared 0.025" matches. May miss the caption methodology (cost index calculation, control variables) or the 2-line title format. | Title (both lines including parenthetical), both axis labels with tick ranges, legend text with R-squared, dashed black regression line, uniform salmon point color (no encoding), scatter shape (elliptical cloud centered at origin, outliers at y=3-4), grid lines, and full caption including cost index definition, control variables, and elasticity interpretation. Parity met. |
| Retrieval Value | "Chart about API costs and usage" with no statistics, no methodology context, no document provenance. Not self-contained. | Self-contained summary citing R-squared = 0.025 and the negative relationship between cost and task prevalence. Searchable for "API cost index usage share" or "R-squared 0.025". Missing the elasticity interpretation and the report's conclusion that cost is a minor factor. | Self-contained annotation embedding the finding that cost explains only 2.5% of usage variance, the -0.29 elasticity interpretation (10% cost increase = ~3% prevalence decrease), the report's conclusion that "other factors beyond cost appear to matter more", the partial regression methodology, and Chapter 3 provenance. Findable by "API cost task prevalence elasticity", "partial regression O*NET tasks", or "Figure 3.9 Anthropic Economic Index". |
